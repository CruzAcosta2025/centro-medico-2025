...
<div style="max-width: 900px; margin: 0 auto; padding: 20px; width: 95%;">
    <div style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        ...
        <div style="background: linear-gradient(to right, #2563eb, #1e40af); padding: 20px;">
            <h2 style="color: #ffffff; font-size: clamp(20px, 4vw, 24px); margin: 0;">Registrar Diagnóstico</h2>
        </div>
        ...
        <div style="padding: 20px;">
            <form action="{{ route('diagnosticos.store', $historial->id_historial) }}" method="POST">
                ...
                <div style="display: flex; justify-content: flex-end; align-items: center; margin-top: 30px; flex-wrap: wrap; gap: 10px;">
                    <a
                        href="{{ route('diagnosticos.index', ['dni' => $paciente->dni]) }}"
                    ...
                    <button
                        type="submit"
                        style="padding: 10px 20px; background-color: #1e40af; color: #ffffff; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; min-width: 120px; font-weight: 600;"
                    >
                        Guardar
                    </button>
                </div>
